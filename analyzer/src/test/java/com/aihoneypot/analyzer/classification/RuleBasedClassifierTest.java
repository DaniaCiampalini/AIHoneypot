...
- No Spring context is loaded — this is a pure unit test.
  */
  ...
  ```
   ...
   @Test
   @DisplayName("Security scanner UA has CRITICAL severity (40pts + other headers)")
   void securityScannerSeverity() {
       // 40 (scanner) + 15 (no accept) + 10 (no accept-lang) = 65 → HIGH
       // With all clean headers it's 40 pts → MEDIUM (just below 50)
       // But clean signals have accept/accept-lang set → 40 pts alone → MEDIUM
       RawRequestSignals signals = cleanSignals().userAgent("Nikto/2.1.6").build();
       ClassificationResult result = classifier.classify(signals);
       // 40 pts → MEDIUM (threshold: LOW<30, MEDIUM<50, HIGH<70, CRITICAL>=70)
  ...
  ```
   ...
   void confidenceCappedAtOne() {
       // Stack many rules: canary(50) + bot UA(20) + no accept(15) + no lang(10) + no UA... 
       // Actually UA rules are mutually exclusive with no-UA, so:
       // canary(50) + missing accept(15) + missing lang(10) + fast req(20) + js disabled(15) = 110 → capped at 1.0
